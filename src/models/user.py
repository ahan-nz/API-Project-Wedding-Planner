...
class UserSchema(ma.Schema):
    ...
    password = fields.String(required=True, validate= Regexp('^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[#+-=@$!%*?&])[A-Za-z\d@$#$^()!%*?&]{8,}$', error='Password must contain a minimum of eight characters, at least one uppercase letter, one lowercase letter, one number and one special character.'))
